...
from PyQt6.QtCore import Qt, QThread, pyqtSignal, QSettings, QObject
...
LIGHT_THEME = """
    ...
    QLineEdit[placeholderText*="Authorization"] {
...
def get_manga_pages(slug_url, volume_number, chapter_number, token):
    ...
    try:
        ...
        return [
            f"https://img3.imglib.info{page['url']}"
...
class DownloadThread(QThread):
    ...
    def __init__(self, slug_url, volume_number, chapter_number, save_directory, token, parent=None):
        super().__init__(parent)
        self.slug_url = slug_url
        self.volume_number = volume_number
        self.chapter_number = chapter_number
        self.save_directory = save_directory
        self.token = token  # Добавляем токен
    ...
    def run(self):
        try:
            ...
            page_urls = get_manga_pages(
                self.slug_url,
                self.volume_number,
                self.chapter_number,
                self.token  # Передаем токен
            )
...
class MangaDownloaderApp(QMainWindow):
    def __init__(self):
        ...
        self.last_save_dir = ""
        self.manga_cache = {}
        self.chapter_threads = []
        self.loader_threads = []
        self.token = ""  # Добавляем атрибут

        # Загрузка сохраненного токена
        settings = QSettings("MangaDownloader", "AppSettings")
        self.token = settings.value("token", "")
    ...
    def init_ui(self):
        ...
        self.token_input = QLineEdit()  # Инициализируем поле ввода
        self.token_input.setPlaceholderText("Authorization Token (Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9...)")
        self.token_input.textChanged.connect(self.update_token)
        token_layout.addWidget(QLabel("Токен:"))
        token_layout.addWidget(self.token_input)
        search_layout.insertLayout(1, token_layout)  # Вставляем после темы

        # Добавить подсказку ПОСЛЕ создания поля
        self.token_input.setToolTip(
            "Получить токен можно через DevTools в браузере:\n"
            "1. Откройте сайт hentailib.me\n"
            "2. Авторизуйтесь\n"
            "3. Откройте Network в инструментах разработчика\n"
            "4. Найдите любой запрос к api2.mangalib.me\n"
            "5. Скопируйте значение из заголовка Authorization"
        )
    ...
    def start_download(self):
        ...
        if not self.token:
            self.log_message("Введите Authorization Token!", "error")
            return
...
class AuthWorker(QObject):
    ...
    def login(self):
        try:
            success, message = self.auth_manager.login(self.email, self.password)
